...
class Tabla:
    ...
    def __init__(self, master, nameTabla, db) -> None:
        ...
        self.desplegar_tablas = ttk.Combobox(
            self.labelframe, state='readonly', font='Verdana 25 normal italic')
        ...
        estilo_tabla.configure("Treeview", font=(
            'courier', 12, 'bold', 'italic'), foreground='black', rowheight=35)
    ...
    def generar_contraseña(self):
        ...
        mayus = ''.join(random.sample(mayus, items['M'])) if 'M' in items else ''
        minus = ''.join(random.sample(minus, items['m'])) if 'm' in items else ''
    ...
    def insertOrUpdateCredencial(self, nameTabla, mode='I'):
        ...
        if not (llave):
            ...
            if mode == 'I':
                self.db.write(self.desplegar_tablas.get(), VALUES)
            ...
            self.rellenar_datos(self.desplegar_tablas.get(), self.db)
            self.frame_add_credencial.pack_forget()
