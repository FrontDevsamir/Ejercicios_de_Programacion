...
class Tabla:
    ...
    def __init__(self, master, nameTabla, db) -> None:
        ...
        self.master.bind('<Control-u>', lambda x: self.event_Update(self.desplegar_tablas.get()))
        self.master.bind('<Control-d>', lambda x: self.event_Delete(self.desplegar_tablas.get()))
        self.popup()
    ...
    def popup(self):
        ...
        self.popup_menu.add_command(label="Mover A Tabla",
                                    command= None, foreground= 'blue', font= '"Cascadia Code PL" 15 normal')
        ...
        # print(len(self.tabla_credenciales.selection()), '----------------------------')
    ...
    def do_popup(self, event):
        try:
            ...
            if len(self.tabla_credenciales.selection()) == 1 :
                ...
                if self.popup_menu.entrycget(6, 'label') == 'Eliminar' :
                    self.popup_menu.add_separator()
                    self.popup_menu.add_separator()
                    self.popup_menu.add_command(label="Duplicar Y Modificar",
                                    command= lambda : self.duplicar_modificar(), foreground='blue', font= '"Cascadia Code PL" 15 normal')
            else :
                try :
                    self.popup_menu.delete(4)
                    self.popup_menu.delete(5)
                    self.popup_menu.delete(6)
                except :
                    pass
    ...
    def duplicar_modificar(self) :
        ...
        self.add_update('Nueva', 'AGREGAR', lambda: self.insertOrUpdateCredencial(self.desplegar_tablas.get()), nameTabla= self.desplegar_tablas.get(), modificar= True)
